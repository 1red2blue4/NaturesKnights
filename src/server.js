...
'/media/ghost.png', '/media/logo.png',
'/media/arrow.png', '/media/arrowSpritesheet.png',
'/media/arrowLeft.png', '/media/arrowRight.png',
'/media/healthCircle.png',
'/media/dream_orphans/dreamorphans.ttf',
'/media/source_sans/SourceSansPro-Regular.ttf',
'/media/Roboto/Roboto-Black.ttf',
'/media/blossom_bird.png', '/media/sea_serpent.png',
'/media/sponge.png', '/media/stupid_snake.png',
'/media/savage_tiger_taken.png',
'/media/chipmonk.png', '/media/monkey.png',
'/media/redRect.png', '/media/naturesKnightsbg.png',
'/media/naturesKnights.wav'];
...
const onJoined = (sock) => {
  ...
  socket.on('awaitBothPlayers', (data) => {
    ...
    // if you are not the first user and the person
    // before you is waiting and you are an even-
    // numbered player, stop waiting
    if (data.ticket !== 1 &&
    waiting[data.ticket - 1] === true && users[data.ticket] % 2 === 0) {
  ...
  socket.on('bothPlayersAvailable', (data) => {
    ...
    io.sockets.in(`room${userInfo.room}`).emit('receiveEnemyTeam',
    { team: userInfo.team, ticket: userInfo.ticket });
  });
  ...
  socket.on('firstTurn', (data) => {
    socket.broadcast.to(`room${data.yourRoomNum}`)
    .emit('nextTurn', { next: data.turnOrder[0],
    newTurnOrder: data.turnOrder });
  });
  ...
  socket.on('takeTurn', (data) => {
    ...
    socket.broadcast.to(`room${data.yourRoomNum}`)
    .emit('nextTurnEnemy', { next: theTurnOrder[0],
    newTurnOrder: theTurnOrder });
    socket.emit('nextTurn', { next: theTurnOrder[0], newTurnOrder: theTurnOrder });
  });

  socket.on('sendDamageUI', (data) => {
    socket.broadcast.to(`room${data.yourRoomNum}`)
    .emit('receiveDamageUI', { damType:
    data.damType, damNum: data.damNum, targetNum:
    data.targetNum, targetType: data.targetType });
  });

  socket.on('recalculateTeams', (data) => {
    socket.broadcast.to(`room${data.yourRoomNum}`)
    .emit('sendTeamsForNewTurn', { theirNewTeam:
    data.yourTeam, myNewTeam: data.enemyTeam });
  });

  socket.on('sendAbilityUI', (data) => {
    socket.broadcast.to(`room${data.yourRoomNum}`)
    .emit('receiveAbilityUI', { abilityType:
    data.abilityType, abilityNum: data.abilityNum,
    targetNum: data.targetNum, targetType: data.targetType });
  });
